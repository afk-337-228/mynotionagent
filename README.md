...
**Бот не отвечает, хотя set_webhook вернул 200:** открой в браузере `https://ТВОЙ_VERCEL_URL/api/webhook_info?secret=ТВОЙ_WEBHOOK_SECRET`. Там будет JSON с полем `result.url` — текущий URL вебхука в Telegram. Он должен быть **ровно** `https://ТВОЙ_ДОМЕН/api/webhook` (домен без опечаток). Если там другой URL — задай в Vercel `PRODUCTION_DOMAIN=твой-домен.vercel.app`, сделай Redeploy и снова открой ссылку set_webhook.

**В webhook_info видишь `last_error_message: "404 Not Found"`:** значит, по адресу `.../api/webhook` Vercel отдаёт 404 и маршрут не находится. Проверь: открой в браузере **GET** `https://ТВОЙ_ДОМЕН/api/webhook` — должна открыться страница с текстом «Notion Telegram Bot webhook endpoint. Use POST.» Если вместо этого 404: в Vercel **Settings** → **General** убедись, что **Root Directory** пустой (корень репо), в **Build and Deployment** не заданы свои **Build Command** и **Output Directory** (для serverless их не нужно). Затем сделай **Redeploy** и снова проверь GET `/api/webhook`.
