...
- **Режим `WHISPER_MODE=local`** (рекомендуется): используется библиотека `faster-whisper` и модель `tiny` (или `base`) на CPU. Полностью бесплатно, работает локально. Для Docker установлен `ffmpeg`.
- Режим API в текущей версии не реализован; при необходимости его можно добавить (например, через OpenAI API).
